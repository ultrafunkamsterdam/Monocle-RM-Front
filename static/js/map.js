...
function initMap() { // eslint-disable-line no-unused-vars
    map = new google.maps.Map(document.getElementById('map'), {
        ...
        zoom: Number(getParameterByName('zoom')) || Store.get('zoomLevel') || 14,
        minZoom: Store.get('minZoomLevel') || minZoomLevel,
...
function searchControl(action) {
    console.log('[invalid] searchControlis disabled')
    $('#scan-here').toggleClass('disabled', true)
}

function updateSearchStatus() {
    console.log('[invalid] updateSearchStatus is disabled')
...
function pokemonLabel(item) {
    ...
    <div class='pokemon name'>
      ${name} <span class='pokemon name pokedex'><a href='http://pokemon.gameinfo.io/en/pokemon/${id}' target='_blank' title='View in Pokédex'>#${id}</a></span> ${formString} <span class='pokemon gender rarity'> ${rarityDisplay}</span> ${typesDisplay}
    </div>`
    ...
    if (atk !== null && def !== null && sta !== null) {
        ...
        contentstring += `
          <div class='pokemon container'>
            <div class='pokemon container content-left'>
              <div>
                ...
                <!--span class='pokemon'>Level: </span><span class='pokemon'>${/*pokemonLevel */null }</span-->
          ...
          <div class='pokemon container content-right'>
            <div>
              ...
              <div class='pokemon'>
                IV <span class='pokemon encounter'>${iv.toFixed(1)} %</span> (A${atk}/D${def}/S${sta})
              </div>
              <div class='pokemon'>
      ...
      <div class='pokemon container content-right'>
        <div>
          ...
          <div class='pokemon'>
            IV: <span class='pokemon no-encounter'>n/a for ${name}</span>
          </div>
          <div class='pokemon'>
            Moveset: <span class='pokemon no-encounter'>n/a for ${name}</span>
          </div>
    ...
    if ((isUpcomingRaid || isRaidStarted) && isRaidFilterOn && isGymSatisfiesRaidMinMaxFilter(raid)) {
        ...
        if (isRaidStarted) {
            ...
            image = `
                ...
                <div class='raid'>
                <span style='font-size:1.3rem;color:rgb(${raidColor[Math.floor((raid.level - 1) / 2)]})'>
                ${levelStr}
                </span>
            ...
            if (raid.pokemon_id !== null && pokemonWithImages.indexOf(raid.pokemon_id) !== -1) {
                image = `<div class='raid'>
                        <div style='font-size:1.3rem;color:rgb(${raidColor[Math.floor((raid.level - 1) / 2)]})'>
                        ${levelStr}
                        </div>
                        <span class='raid countdown label-countdown' disappears-at='${raid.end}'></span> left
                        </div>
...
function updateGymMarker(item, marker) {
    ...
    const pokemonWithImages = [
        3, 6, 9, 59, 65, 68, 89, 94, 103, 110, 112, 125, 126, 129, 131, 134,
        135, 136, 143, 144, 145, 146, 150, 151, 153, 156, 159, 248, 249
    ]
    if (item.raid !== null && isOngoingRaid(item.raid) && Store.get('showRaids') && raidLevel >= Store.get('showRaidMinLevel') && raidLevel <= Store.get('showRaidMaxLevel')) {
        ...
        if (pokemonWithImages.indexOf(item.raid.pokemon_id) !== -1) {
            ...
            markerImage = 'static/images/raid/' + item['raid']['pokemon_id'] + '.png'
        }
        marker.setIcon({
            url: markerImage,
            ...
            pokemonHtml = `
                <center>
                    ...
                    <p style="font-size: .75em; margin: 5px;">
                        No additional gym information is available for this gym. 
                    </p>
                </center>
            `
        }
